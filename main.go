...
func main() {
	logger.Logger.Printf("main(): Iniciando aplicação")

	if err := run(); err != nil {
		logger.Logger.Printf("main(): ERRO - %v", err)
		_ = displayPopup("Error: " + err.Error())
		os.Exit(1)
	}

	logger.Logger.Printf("main(): Finalizado com sucesso")
}
...
func run() error {
	...
	showSuccess(text)
	logger.Logger.Printf("run(): Sucesso exibido ao usuário")
...
func showSuccess(text string) {
	...
	logger.Logger.Printf("showSuccess(): Mensagem preparada, tamanho: %d caracteres", len(msg))
